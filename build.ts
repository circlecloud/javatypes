...
function convertJson2TypeDefiend(infile: string, outDir: string) {
    ...
    temp += `${formatDoc(obj.docString, closeBuk)}`
    ...
    if (isInterface) {
        if (safeInterface.length > 0) {
            temp += ' extends '
            temp += safeInterface.map(ifs => formatClass(ifs)).join(', ')
        }
    } else {
        temp += `${(obj.superclass) ? (' extends ' + formatClass(obj.superclass)) : ''}`
        if (safeInterface.length > 0) {
            temp += ' implements '
            temp += safeInterface.map(ifs => formatClass(ifs)).join(', ')
        }
...
//{ qualifiedName: string, name: string, parameterizedType: object }
function formatClass(type: any, gType = "<T>") {
    let className = type.type == "<any>" ? "object" : type.type
...
function formatParameters(params: any[]) {
    ...
    tempParam += `${lastParam.dimension == "[]" ? "..." : ""}${mappingName(lastParam.name)}: ${mappingType(lastParam.type)}`
    return tempParam
}
...
function mappingType(type: JavaType, isParam: boolean = true): string {
    ...
    let javaType = formatClass(type)
    let tsType = javaType.replace(type.qualifiedName, outType).replace('function', 'function$')
    return isParam && javaType !== tsType && type.type.includes('.') ? `${javaType} | ${tsType}` : tsType
}
...
for (const file of files) {
    let fileName = convertJson2TypeDefiend(file, outDir)
    if (fileName) {
        index += `/// <reference path="./${fileName}" />\n`
    }
}
